package com.csl.ws.hotel.test;


import java.util.List;

import org.apache.commons.lang3.RandomUtils;
import org.apache.log4j.Logger;
import org.junit.After;
import org.junit.Before;
import org.junit.Test;
import org.springframework.context.ApplicationContext;
import org.springframework.context.support.ClassPathXmlApplicationContext;

import com.csl.ws.hotel.po.AuthInfo;
import com.csl.ws.hotel.po.Hotel;
import com.csl.ws.hotel.service.AuthService;
import com.csl.ws.hotel.service.HandlerService;
import com.csl.ws.hotel.service.HotelService;
import com.csl.ws.hotel.util.Cryptos;
import com.csl.ws.hotel.util.DateUtils;

public class AuthTest {
	ApplicationContext ctx=null;
	HotelService hotelService=null;
	AuthService authService=null;
	HandlerService handlerService=null;
	Logger logger=Logger.getLogger(AuthTest.class);
	
	@Before
	public void setUp() throws Exception {
		ctx=new ClassPathXmlApplicationContext("beans.xml");
		System.out.println("context:"+ctx);
		hotelService=(HotelService) ctx.getBean("hotelService");
		authService=(AuthService) ctx.getBean("authService");
		handlerService=(HandlerService) ctx.getBean("handlerService");
	}

	@Test
	public void testSelectHotel(){
		String lgzt="1";
		List<Hotel> list=hotelService.findHotelsByZt(lgzt);
		logger.info("旅馆数量："+list.size());
	}
	
	@Test
	public void testInitAuth(){
		List<Hotel> list=hotelService.findHotelsByZt("1");
		for(Hotel hotel:list){
//			String hotelstr=hotel.getCode()+"&"+DateUtils.getDateTime();
			String hotelstr=hotel.getCode()+"&"+RandomUtils.nextInt();
			String authCode=Cryptos.aesEncrypt(hotelstr);
			String authDate=DateUtils.getDateTime();
			
			AuthInfo authInfo=new AuthInfo(hotel.getCode(), authCode, authDate, 1);
		//	System.out.println(authInfo);
			
			int res=authService.saveAuthInfo(authInfo);
			logger.info("旅馆["+hotel.getCode()+"]认证授权结果："+res);
			
		}
	}
	
	/**
	 * 单个旅馆进行授权
	 */
	@Test
	public void testGenHotelAuth(){
		//此处手动传入要授权的旅馆编码10位，如：4101026008
		String hotelCode="4101026008";
		
		String hotelstr=hotelCode+"&"+RandomUtils.nextInt();
		String authCode=Cryptos.aesEncrypt(hotelstr);
		String authDate=DateUtils.getDateTime();
		
		AuthInfo authInfo=new AuthInfo(hotelCode, authCode, authDate, 1);
	//	System.out.println(authInfo);
		
		int res=authService.saveAuthInfo(authInfo);
		logger.info("旅馆["+hotelCode+"]认证授权结果："+res);
	}
	
	
	
	@Test
	public void testMd5(){
		//4101026004,4101026007,4101026008
		String instr="4101052039&20181230";
	//	String md5str=Md5Crypt.md5Crypt(instr.getBytes());
	//	System.out.println(md5str); //$1$TvxJtCm2$d0e.yLQBZzAHlxrC8E1cA1
		
		/**
		byte[] byes=Digests.md5(instr.getBytes());
		String md5str2=new String(byes, 0, byes.length);
		System.out.println(md5str2);
		*/
		String aesstr=Cryptos.aesEncrypt(instr);//16ba0b414b80c93c160651b207ddffa2
		logger.info(aesstr);
		
		//解析认证码为字符串
		//aesstr="f05cd524ccb48a21debf8da3227b2afaed9de1cefde699101d545f02c76e6033";
		aesstr="86c272fc5c6176ea76faeac7ef39201f197d6911c4a3185b460857c9ec773585";
		String destr=Cryptos.aesDecrypt(aesstr);
		
		System.out.println(destr);
		
	}
	//\"type\":\"CI\",
	String jsonStr="{\"ryxx\":{\"id\":\"4101050000160530232603\",\"name\":\"刘佳\","
			+ "\"sex\":\"2\",\"nation\":\"HA\", \"bdate\":\"19790809\",\"zjzl\":\"11\","
			+ "\"zjhm\":\"410119814356744895\",\"xzqh\":\"410105\",\"address\":\"郑州市金水区东风路蓝宝万145\","
			+ "\"inTime\":\"201801262049\",\"noRoom\":\"6129\","
			+ "\"traTime\":\"201801272358\","
			+ "\"rksj\":\"201801102258\",\"hotelId\":\"4101050000\"},"
			+ "\"auth\":{\"hotelId\":\"4101050000\","
				+ "\"authCode\":\"934b9c4520a01c8abc0641c713b5638685d1084a2d7e98875974922c932ad3e9\"},"
			+ "\"tpxx\": [{"
				+ "\"ryid\":\"4101050000160530232603\","
				+ "\"photo\": \""
				+ "\","
				+ "\"flag\":1 },{"
				+ "\"ryid\":\"4101050000160530232603\","
				+ "\"photo\": \""
				+ "\","
				+ "\"flag\":2}]" 
			+ "}";
	
	
	@Test
	public void testHandleInfo(){
		boolean result=handlerService.validateUpinfo(jsonStr);
		System.out.println("验证结果："+result);
		if (result) {
			String retmsg=handlerService.handleCIinfo(jsonStr);
			System.out.println(retmsg);
		}else {
			System.err.println("数据有问题，请检查");
		}
	}
	
	@Test
	public void testCC(){
		String upjsonStr = "{"
				+ "\"tsxx\":{\"id\":\"4101030292160530232635\",\"tssj\":\"201802171028\"}"
				+ "}";
		String ccret=handlerService.handleCCinfo(upjsonStr);
		System.out.println(ccret);
		
	}
	@After
	public void tearDown() throws Exception {
		
	}
	
	

}
